...
data {
  ...
  matrix[S,S] cho_state_covariance; //Cov of State
}
...
transformed parameters {
  ...
  // Set log-odds for the last day
    ...
    logit_p[:, T] = cho_state_covariance * e_T;

  // Define the random walk for the preceding days (T-1 to 1)
  for (t in 1:(T-1)) {
      logit_p[:, T-t] = cho_state_covariance * e_t[:, T-t] + logit_p[:, T-t+1];  // Log-odds depend on the next day
  }
